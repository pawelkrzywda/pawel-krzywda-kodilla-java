...
class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask(){
        ...
        //When
        List<TaskList> tasksInitiated = new ArrayList<>();
        tasksInitiated.add(new TaskList("In progress"));

        long quantity = project.getTaskLists().stream()
                .filter(tasksInitiated::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .count();
        ...
        long sum = project.getTaskLists().stream()
                ...
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated)
                .collect(Collectors.summingLong(d->DAYS.between(d, LocalDate.now())));
    ...
    void testAddTaskListAverageWorkingOnTaskVer2(){
        ...
        List<Long> dates = project.getTaskLists().stream()
                .filter(tasksInitiated::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated)
        ...
        OptionalDouble average = LongStream.range(0, dates.size())
                .map(n ->  dates.get((int) n))
                .average();

        //Then
        Assertions.assertEquals(10.0, average.getAsDouble());
    }
